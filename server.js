...
class FootballRoom extends Room {
  ...
  onCreate(options) {
    ...
    this.onMessage("chat", (client, msg) => { const s = this.state.players.get(client.sessionId)?.name || "Unknown"; this.broadcast("chat", { sender: s, text: (msg || "").substring(0, 200) }); });
  ...
  onJoin(client, options) {
    ...
    player.x = isP1 ? 150 : 820; player.y = 415; player.color = isP1 ? "#ff00ff" : "#00f2ff"; player.side = isP1 ? "left" : "right";
  ...
  onLeave(client) {
    ...
    if (!player) return;
    player.reconnecting = true; player.disconnectTime = Date.now();
    this.broadcast("opponentReconnecting", { sessionId: client.sessionId });
    this.reconnectTimers[client.sessionId] = setTimeout(() => { if (player.reconnecting) { this.state.players.delete(client.sessionId); this.broadcastPlayerInfo(); this.broadcast("playerLeft", {}); } }, 30000);
  }

  broadcastPlayerInfo() {
    const p1 = [...this.state.players.values()].find(p => p.side === "left");
    const p2 = [...this.state.players.values()].find(p => p.side === "right");
    this.broadcast("playerNames", { p1: p1?.name || "—", p2: p2?.name || "—", p1Ready: p1?.ready || false, p2Ready: p2?.ready || false, password: this.state.password });
  }

  startCountdown() {
    this.state.matchState = "countdown"; this.state.countdown = 3;
    this.broadcast("countdown", { value: this.state.countdown });
    const interval = setInterval(() => { if (this.state.matchState !== "countdown") { clearInterval(interval); return; } this.state.countdown--; if (this.state.countdown <= 0) { clearInterval(interval); this.state.matchState = "live"; this.broadcast("gameStarted"); this.broadcast("event", { type: "MUSIC_NEXT" }); } else { this.broadcast("countdown", { value: this.state.countdown }); } }, 1000);
  }

  gameTick() {
    if (this.state.matchState !== "live" || this.state.gameOver || this.state.players.size < 2) return;
    if (this.state.goalFreeze > 0) { this.state.goalFreeze--; if (this.state.goalFreeze === 0) this.broadcast("event", { type: "FREEZE_END" }); return; }
    const FIXED_DT = 1 / 30; const ball = this.state.ball;
    this.state.players.forEach((player, sid) => {
      const input = this.inputs[sid] || {};
      const dx = player.x + 15 - ball.x, dy = player.y + 32 - ball.y, hasBall = dx * dx + dy * dy < 2500;
      if (hasBall && (input.shoot || input.turbo)) { player.vx = 0; const speed = input.turbo ? 45 : 20; ball.vx = player.side === "left" ? speed : -speed; if (input.up && !input.down) ball.vy = -14; else if (input.down) ball.vy = 10; else ball.vy = -2; this.broadcast("event", { type: "SHOT", data: { turbo: input.turbo, color: player.color } }); }
      else { const accel = 0.6; if (input.left) player.accelX -= accel; if (input.right) player.accelX += accel; if (!input.left && !input.right) player.accelX *= 0.85; player.accelX = Math.min(Math.max(player.accelX, -3), 3); player.vx += player.accelX; player.vx *= 0.9; if (input.up && !player.isJumping) { player.vy = -14; player.isJumping = true; } if (input.down) player.vy += 1; }
    });
    ball.x += ball.vx * FIXED_DT * 60; ball.y += ball.vy * FIXED_DT * 60; ball.vy += 0.25 * FIXED_DT * 60; ball.vx *= 0.995;
    if (ball.y > 480) { ball.y = 480; ball.vy *= -0.7; } if (ball.y < 10) { ball.y = 10; ball.vy *= -0.7; }
    [{ x: 5, k: this.state.keeper1 }, { x: 983, k: this.state.keeper2 }].forEach(({ x: kx, k }) => { if (ball.x + 10 > kx && ball.x - 10 < kx + 12 && ball.y + 10 > k.y && ball.y - 10 < k.y + 60) { if (Math.abs(ball.vx) > 25) this.broadcast("event", { type: "SHOT", data: { turbo: false, color: "#fff" } }); ball.vx *= -1.1; ball.x = kx < 500 ? 25 : 970; } });
    this.state.players.forEach(p => { if (ball.x + 10 > p.x && ball.x - 10 < p.x + 30 && ball.y + 10 > p.y && ball.y - 10 < p.y + 65) { const rvx = ball.vx - p.vx, rvy = ball.vy - p.vy; ball.vx = p.vx - rvx * 0.6; ball.vy = p.vy - rvy * 0.6; ball.x = ball.x < p.x + 15 ? p.x - 11 : p.x + 31; } });
    if (ball.x < 0 || ball.x > 1000) { if (ball.y > 150 && ball.y < 350) { if (ball.x < 0) this.state.p2Score++; else this.state.p1Score++; this.broadcast("event", { type: "GOAL", data: { scorer: ball.x < 0 ? "p2" : "p1", color: ball.x < 0 ? "#00f2ff" : "#ff00ff" } }); this.state.goalFreeze = 60; ball.x = 500; ball.y = 250; ball.vx = (Math.random() > 0.5 ? 5 : -5); ball.vy = -3; if (this.state.p1Score >= this.targetGoals || this.state.p2Score >= this.targetGoals) { this.state.gameOver = true; this.state.matchState = "end"; this.state.winnerMessage = this.state.p1Score >= this.targetGoals ? "Player 1 Wins!" : "Player 2 Wins!"; this.state.lastWinner = this.state.p1Score >= this.targetGoals ? "p1" : "p2"; } } else { ball.vx *= -1; ball.x = ball.x < 0 ? 5 : 995; } }
    const targetY = ball.y - 30;
    [this.state.keeper1, this.state.keeper2].forEach((k, i) => { k.vy += (targetY - k.y) * (i === 0 ? 0.12 : 0.1); k.vy *= 0.7; k.y += k.vy * FIXED_DT * 60; k.y = Math.min(295, Math.max(155, k.y)); });
    this.state.players.forEach(p => { p.vy += 0.7; p.x += p.vx * FIXED_DT * 60; p.y += p.vy * FIXED_DT * 60; p.vx *= 0.85; if (p.y > 415) { p.y = 415; p.vy = 0; p.isJumping = false; } p.x = Math.min(930, Math.max(40, p.x)); });
    if (this.state.timeLeft > 0) { this.state.timeLeft -= FIXED_DT; if (this.state.timeLeft <= 0) { this.state.gameOver = true; this.state.matchState = "end"; this.state.winnerMessage = this.state.p1Score > this.state.p2Score ? "Player 1 Wins!" : (this.state.p2Score > this.state.p1Score ? "Player 2 Wins!" : "Draw!"); this.state.lastWinner = this.state.p1Score > this.state.p2Score ? "p1" : (this.state.p2Score > this.state.p1Score ? "p2" : "draw"); } }
  }
}
