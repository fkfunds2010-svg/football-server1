...
// ---------- Schemas (exactly as you have them) ----------
...
// ---------- Room (unchanged, with the 100ms delay fix) ----------
class FootballRoom extends Room {
  ...
  onJoin(client, options) {
    ...
    // ✅ Delay to let the client register its message handlers before broadcasting
  ...
  onLeave(client) { const player = this.state.players.get(client.sessionId); if (!player) return; player.reconnecting = true; player.disconnectTime = Date.now(); this.broadcast("opponentReconnecting", { sessionId: client.sessionId }); this.reconnectTimers[client.sessionId] = setTimeout(() => { if (player.reconnecting) { this.state.players.delete(client.sessionId); this.broadcastPlayerInfo(); this.broadcast("playerLeft", {}); } }, 30000); }
  broadcastPlayerInfo() { const p1 = [...this.state.players.values()].find(p => p.side === "left"); const p2 = [...this.state.players.values()].find(p => p.side === "right"); this.broadcast("playerNames", { p1: p1?.name || "—", p2: p2?.name || "—", p1Ready: p1?.ready || false, p2Ready: p2?.ready || false, password: this.state.password }); }
...
// ---------- Matchmaking routes (match your client exactly) ----------
app.post("/matchmake/create/:roomName", async (req, res) => {
  try {
    const { roomName } = req.params;
    const options = req.body || {};
    const room = await gameServer.matchmaker.create(roomName, options);
